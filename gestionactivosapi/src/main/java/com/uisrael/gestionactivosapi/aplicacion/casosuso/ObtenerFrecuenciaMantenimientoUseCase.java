 ...
 *
 * <p><b>Decisión de diseño:</b> La columna {@code frecuencia_dias} fue eliminada de
 * {@code mantenimientos} porque era redundante con
 * {@code mantenimientos_programados.frecuencia_dias}. La frecuencia se resuelve así:</p>
 * <ul>
 *   <li>Si el mantenimiento tiene {@code fk_programado} → se obtiene del programado.</li>
 *   <li>Si es manual (sin fk_programado) → retorna {@code Optional.empty()} (no tiene frecuencia).</li>
 * </ul>
 */
...
public class ObtenerFrecuenciaMantenimientoUseCase {
    ...
    /**
     * Obtiene la frecuencia del mantenimiento resolviendo desde el programado asociado.
     *
     * @param idMantenimiento ID del mantenimiento
     * @return resultado con frecuencia si es programado, empty-frequency si es manual
     * @throws RecursoNoEncontradoException si el mantenimiento no existe
     */
    public Resultado ejecutar(Integer idMantenimiento) {
        MantenimientosJpa mantenimiento = mantenimientoRepo.findById(idMantenimiento)
        ...
        if (mantenimiento.getFkProgramado() != null && mantenimiento.getProgramadoRel() != null) {
            return new Resultado(
                mantenimiento.getProgramadoRel().getFrecuenciaDias(),
                true
            );
        }
