...
public class CustodiasControlador {
	...
	public String listarCustodias(Model model) {
	    ...
	    java.util.function.Function<CustodiasResponseDTO, String> actaKeyFn = x -> {
	        int idC = (x != null && x.getFkCustodio() != null)
	                ? x.getFkCustodio().getIdCustodio()
	                : (x != null ? x.getIdCustodio() : 0);
	        String tipo = (x != null && x.getTipoMovimiento() != null)
	                ? x.getTipoMovimiento() : "ASIGNACION";
	        String fecha = (x != null && x.getFechaInicio() != null)
	                ? x.getFechaInicio().toString() : "sin-fecha";
	    ...
	    List<ActaResumen> actas = actaDetalles.entrySet().stream()
	            .map(entry -> {
	                ...
	                CustodiasResponseDTO first = items.stream()
	                        ...
	                        .orElse(items.get(0));
	                int idC = first.getFkCustodio() != null
	                        ? first.getFkCustodio().getIdCustodio() : first.getIdCustodio();
	                String tipo = first.getTipoMovimiento() != null
	                        ? first.getTipoMovimiento() : "ASIGNACION";
	                ...
	                LocalDate fechaFin = items.stream()
            ...
            .sorted(Comparator.comparingInt(ActaResumen::getMinPk))
            .toList();
...
private String procesarTrasladoDesdeFormulario(CustodiasRequestDTO custodia, Model model, HttpSession session) {
    ...
    boolean hayMismoCustodio = custodia.getEquiposSeleccionados().stream()
            .map(activaPorEquipo::get)
            .filter(x -> x != null)
            .anyMatch(x -> {
    ...
    if (hayMismoCustodio) {
        cargarFormularioNuevaCustodia(model, custodia,
                "Uno o varios equipos ya pertenecen al custodio destino.");
        return "Custodias/nuevoCustodia";
    }
...
private String procesarBajaDesdeFormulario(CustodiasRequestDTO custodia, Model model, HttpSession session) {
    ...
    LocalDate fechaBaja = (custodia.getFechaFin() != null)
            ? custodia.getFechaFin()
    ...
    List<EquiposResponseDTO> equiposBaja = equiposProcesados.stream()
...
public String mostrarCerrarCustodia(@PathVariable Integer idCustodio,
...
public String cerrarCustodia(@ModelAttribute("form") CustodiasRequestDTO form,
    ...
    List<CustodiasResponseDTO> seleccionadas = servicioCustodias.listarCustodias().stream()
            .filter(CustodiasResponseDTO::isEstado)
            .filter(c -> c.getIdCustodiaEquipo() > 0 && form.getDetallesEntregados().contains(c.getIdCustodiaEquipo()))
            .filter(c -> getIdCustodio(c) != null && getIdCustodio(c).equals(form.getIdCustodio()))
            .toList();
    ...
    String observacion = (form.getObservacion() == null || form.getObservacion().isBlank())
            ? "Salida de activos"
...
public String verActaEntrega(Model model, HttpSession session) {
  ...
  if (tipoMov == null || tipoMov.isBlank()) tipoMov = "ASIGNACION";
...
public String reenviarCorreoActa(@PathVariable Integer idCustodio,
                                 @RequestParam(required = false) String tipo,
                                 @RequestParam(required = false) String fecha,
                                 RedirectAttributes redirectAttributes) {
    List<CustodiasResponseDTO> lista = filtrarPorActa(idCustodio, tipo, fecha);
...
public String verActaEntregaActa(@PathVariable Integer idCustodio,
                                 @RequestParam(required = false) String tipo,
                                 @RequestParam(required = false) String fecha,
                                 Model model) {
...
private void enviarCorreoActaEntrega(List<CustodiasResponseDTO> lista, byte[] pdfBytes, String tipoMov) {
    ...
    String nombre = cab.getFkCustodio().getNombre() != null
            ? cab.getFkCustodio().getNombre() : "Custodio";
...
private String custodioNombre(CustodiasResponseDTO x) {
    if (x == null || x.getFkCustodio() == null) return "";
...
private void cargarFormularioNuevaCustodia(Model model, CustodiasRequestDTO custodia, String error) {
    ...
    var cargosActivos = servicioCargo.listarCargos().stream()
            .filter(c -> c.isEstado()).toList();
    var ubicacionesActivas = servicioUbicacion.listarUbicaciones().stream()
    ...
    // Mapa equipoId -> custodioId (para filtrar equipos por custodio origen en traslado)
    Map<Integer, Integer> equipoCustodioMap = servicioCustodias.listarCustodias().stream()
        ...
        .collect(Collectors.toMap(
            x -> x.getFkEquipo().getIdEquipo(),
            x -> x.getFkCustodio().getIdCustodio(),
            (a, b) -> a
        ));
...
private Set<Integer> obtenerIdsEquiposConCustodiaActiva() {
    return servicioCustodias.listarCustodias().stream()
            ...
            .filter(e -> e != null && e.getIdEquipo() > 0)
...
private CustodiosResponseDTO buscarCustodioOrigenTraslado(List<CustodiasResponseDTO> listaTraslado) {
    if (listaTraslado == null || listaTraslado.isEmpty()) return null;

    Set<Integer> equipoIds = listaTraslado.stream()
            .filter(x -> x.getFkEquipo() != null)
            .map(x -> x.getFkEquipo().getIdEquipo())
            .collect(Collectors.toSet());

    LocalDate fechaTraslado = listaTraslado.get(0).getFechaInicio();
    int idCustodioDestino = listaTraslado.get(0).getFkCustodio() != null
            ? listaTraslado.get(0).getFkCustodio().getIdCustodio() : 0;

    return servicioCustodias.listarCustodias().stream()
            .filter(x -> x != null && !x.isEstado()
                    && x.getFkEquipo() != null
                    && equipoIds.contains(x.getFkEquipo().getIdEquipo())
                    && x.getFkCustodio() != null
                    && x.getFkCustodio().getIdCustodio() != idCustodioDestino
                    && (fechaTraslado == null || fechaTraslado.equals(x.getFechaFin())))
            .findFirst()
            .map(CustodiasResponseDTO::getFkCustodio)
            .orElse(null);
}
