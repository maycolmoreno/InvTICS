...
8. Mantener UI Enterprise limpia y sin formularios redundantes — **auditoría completa (2026-07-10), remediación en curso.** ~10 pantallas cumplen (Custodias, Activos, Mantenimiento/notificaciones, Dashboard, Stock, Traslados). Violaciones restantes, de mayor a menor prioridad:
   ...
   * Patrón "página fija en vez de drawer" — **Custodios y Cargos migrados (2026-07-10)**, resto pendiente: Equipos, Roles, Usuarios, Marcas, Departamentos, Categorías de equipo, Ubicaciones, Checklist. Mismo patrón en ambos: `nuevoX.html`/`editarX.html` eliminados, fusionados en un único `<aside class="cui-drawer">` dentro de `listarX.html`, precargado desde `data-*` en la fila (mismo patrón que Stock/Traslados). Rutas `GET .../nuevo-x` y `.../editar-x/{id}` se conservan como redirects de compatibilidad. Errores de validación del servidor pasan a mostrarse como toast (primer error) en vez de mensaje por campo. **Bug de CSRF encontrado y corregido en Cargos** (mismo patrón que en Planificación): el botón "Desactivar" no tenía token CSRF y hubiera dado 403 — vale la pena revisar si Marcas/Departamentos/Categorías/Ubicaciones tienen el mismo gap cuando se migren.
