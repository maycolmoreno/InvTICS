...
8. Mantener UI Enterprise limpia y sin formularios redundantes — **auditoría completa (2026-07-10), remediación mayormente completa (2026-07-12).** Las 10 entidades del patrón drawer + los 3 reportes agrupados + importar equipos + los 2 fragments compartidos con Bootstrap crudo, todos migrados. Pendiente real: pantallas nunca auditadas/migradas en esta ronda (`Custodias/*`, `Inventario/bajas.html`/`compras.html`/`gestionarOC.html`, `roles/permisos.html`, `ubicacionesTecnicos/*`, `reportes/reporte-mantenimientos.html`) y el summary strip de `Inventario/reparaciones.html`. Violaciones ya resueltas:
