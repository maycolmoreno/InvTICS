...
8. Mantener UI Enterprise limpia y sin formularios redundantes — **auditoría completa (2026-07-10), remediación completa de las 14 pantallas auditadas (2026-07-12).** Las 10 entidades del patrón drawer + los 3 reportes agrupados + importar equipos + los 2 fragments compartidos con Bootstrap crudo + limpieza cosmética de 11 pantallas más (incluida la reescritura completa de `ubicacionesTecnicos/historial.html`/`tiempoReal.html`), todas migradas. Pendiente real: el summary strip de `Inventario/reparaciones.html` (no estaba en el alcance de esta auditoría) y `error/backend-error.html` (excepción deliberada, no deuda — ver nota abajo). Violaciones ya resueltas:
   ...
   * **Auditoría completa de las 14 pantallas nunca migradas (2026-07-12)**, hecha con un subagente de exploración de solo lectura. Detalle y estado tras la limpieza cosmética del mismo día:
     ...
     * ~~`ubicacionesTecnicos/historial.html` y `tiempoReal.html`~~ — **Hecho (2026-07-12), reescritura completa** (eran 0% `cui-*`, la deuda más pesada de las 14). `page-header`/`cui-admin-header`, `cui-card` para el mapa y la tabla, `cui-table`, `cui-badge`, `cui-btn`, `fragments/components/empty-state` para los estados vacíos. La lógica de Leaflet (dibujo de marcadores, trayectoria por técnico con colores, click en fila centra el mapa) se dejó **100% intacta** — solo se tocó el HTML/CSS alrededor, no el JS de mapas. Se conservaron como CSS scoped propio (no violación, no hay equivalente `cui-*`): la altura fija del contenedor del mapa (`#mapHistorial`/`#mapTiempoReal`, Leaflet la necesita), `.tecnico-card` (hover + cursor, referenciada por los click-handlers del JS), `.status-dot`/`.hora-label`/`.tiempo-label` (indicadores propios de esta pantalla). Verificado en vivo: ambas cargan 200, el empty-state se ve correctamente sin datos GPS de prueba.
     * `error/backend-error.html` — **excepción deliberada, no deuda.** Es una página standalone que **no carga `components.css`** (donde viven las clases `cui-*`), así que aplicar esas clases ahí dejaría los botones sin estilo. No se toca salvo que se decida agregarle esa dependencia CSS explícitamente.
     * Verificado en vivo (2026-07-12): las 11 pantallas modificadas cargan con 200 y sin errores de Thymeleaf. 20/20 tests del BFF siguen pasando. Con esto, las 14 pantallas de la auditoría quedan cerradas.
...
9. Tests básicos por cada cambio funcional — **hueco real, remediación en curso desde 2026-07-12.** Bien cubierto solo en Compras/OC/Recepción e Inventario Operacional (BFF). **Backend (`gestionactivosapi`): 4 de 23 controladores con test.**
   ...
   * Quedan 19 controladores backend sin test. BFF: **5 de 29 controladores con test** desde 2026-07-25 (`MantenimientoControlador` sumado — ver punto 11). Además, esta misma sesión sumó los primeros tests a nivel de **servicio** en backend (no controlador): `MantenimientoArchivoService`, `MantenimientoInformeService`, `MantenimientoManualService` — ver punto 11.
10. Resiliencia BFF:
...
11. Auditoría de bugs y lógica de negocio — Mantenimiento y Reportería — **Hecho (2026-07-25).** Auditoría del módulo de mantenimientos (OT manual, plan preventivo programado, scheduler de notificaciones) y su reportería (`/reportes/mantenimientos`, Excel, PDF), a pedido explícito del usuario. La reportería en sí (`ReportesControlador`, `MantenimientosExcelService`) no tenía bugs — filtros consistentes entre pantalla y Excel. Se encontraron y corrigieron 2 bugs serios y 3 medianos en el ciclo de vida de la OT, en 5 fases aprobadas una por una:
    * **Fase A — path traversal en evidencias.** `MantenimientoArchivoService.guardarImagenes()` (backend) usaba el nombre de archivo enviado por el cliente (`getOriginalFilename()`) sin sanear al construir la ruta de destino — un nombre `../../algo` escribía fuera de la carpeta de evidencias. Corregido con `sanearNombreArchivo()` (recorta al nombre base, reemplaza todo carácter fuera de `[a-zA-Z0-9._-]+` — el mismo patrón que ya exige el endpoint que sirve las imágenes — y cae a `"evidencia"` si el resultado queda vacío) más una verificación `normalize()`/`startsWith()` como defensa adicional, igual al criterio que ya tenía `leerImagen()` para lectura. De paso resuelve que evidencias con espacios/tildes quedaran inaccesibles vía web (el nombre guardado ahora siempre es servible). 4 tests nuevos (`MantenimientoArchivoServiceTest`, primer test de este servicio).
    * **Fase B — doble cierre de OT.** `MantenimientoManualService.cerrar()` no verificaba que la OT no estuviera ya `CERRADO`: un doble click o un reintento del BFF podía re-ejecutar el cierre completo (sobrescribir resultado/fecha/cerradoPor, volver a correr el plan preventivo, reenviar el correo con PDF, re-evaluar baja recomendada). Corregido lanzando `MantenimientoNoModificableException` (422, excepción de dominio ya existente en el proyecto) si `estadoInterno == CERRADO`. 4 tests nuevos (`MantenimientoManualServiceCerrarTest`, primer test de este servicio).
    * **Fase C — bypass de validación custodio/equipos en modo FARMACIA.** `MantenimientoControlador.guardar()` (BFF) solo validaba que los equipos pertenecieran al custodio cuando `modoSeleccion=ADMINISTRATIVO`; en modo FARMACIA esa regla vivía solo en el JS del formulario, así que un POST manipulado podía registrar una OT con equipos y custodio de sucursales distintas. Corregido con `custodioYEquiposPertenecenASucursal()`, que replica en servidor el mismo criterio que ya aplicaba `refreshEquipoOptionsSucursal()` en el cliente: un equipo es "de la sucursal" si su custodia activa apunta a **cualquier** custodio de esa sucursal, no solo al seleccionado (varios custodios de una misma farmacia pueden compartir equipos, así que la validación 1-a-1 estricta del modo ADMINISTRATIVO no aplica aquí). 4 tests nuevos (`MantenimientoControladorFarmaciaTest`, primer test de `MantenimientoControlador`).
    * **Fase D — datos runtime versionados en git.** `gestionactivosapi/data/mantenimientos/**` (PDFs e imágenes de evidencia reales) estaba trackeado en git, y el `.gitignore` tenía una entrada duplicada (`run-local.ps1` dos veces) y una entrada sin efecto (`.gitignore` ignorándose a sí mismo, ya trackeado desde antes de esa línea). Se hizo `git rm --cached` sobre `gestionactivosapi/data/` (archivos conservados en disco, solo removidos del índice) y se agregó `gestionactivosapi/data/` al `.gitignore`. **Sin resolver, señalado al usuario:** `run-local.ps1` sigue tanto trackeado como listado en `.gitignore` (contradictorio, sin efecto real — ignorar un archivo ya trackeado es un no-op) — decisión de producto (¿compartido entre devs o local-only, dado que tiene un path de Java hardcodeado?) no tomada en esta sesión.
    * **Fase E — PDF cacheado queda obsoleto si falla la regeneración al cerrar.** `MantenimientoInformeService.obtenerOGenerarPdf()` confía en la sola existencia del archivo cacheado; `generarGuardarYEnviar()` (invocado al cerrar la OT) ya sobrescribe el caché con contenido actualizado *antes* de intentar el envío de correo, así que un fallo de envío no dejaba nada obsoleto — pero si la generación misma del PDF fallaba (ej. imagen ilegible, error de iText), el archivo cacheado de antes del cierre (contenido "en proceso", sin resultado técnico) quedaba sirviéndose para siempre sin que nadie lo notara salvo revisando logs. Corregido: `generarYGuardarPdf()` invalida el caché (`MantenimientoArchivoService.eliminarPdfSiExiste()`, nuevo) si `generarInforme()` falla, forzando una regeneración real en la próxima consulta en vez de servir contenido obsoleto en silencio. 4 tests nuevos (`MantenimientoInformeServiceTest`, primer test de este servicio) + 2 tests nuevos en `MantenimientoArchivoServiceTest`.
    * **Pendiente, documentado pero deliberadamente sin corregir (menor/latente, no urgente):**
      * Checklist filtrado por categoría de equipo: el formulario BFF exige marcar *todas* las actividades de `listarActivas()`, pero `MantenimientoManualService.guardarActividades()` valida contra el checklist filtrado por categoría del equipo (`findActivasPorCategoria`). Hoy funciona porque `checklist_categoria` está vacía (cae al fallback de "todas") — ver `project_checklist_categoria_roto.md`; el día que se vincule esa relación, crear OT desde el formulario fallará con "La actividad X no pertenece al checklist activo".
      * `MantenimientoScheduler.verificarMantenimientosPendientes()` (cron 7am) notifica cada día todo plan vencido sin verificar si el equipo ya tiene una OT `EN_PROCESO` — spam operativo.
      * `MantenimientoProgramadoService.obtenerVencidosYProximos()` corta en `hoy.minusYears(1)`: un plan vencido hace más de un año deja de listarse en silencio.
      * `actividadIds` es `@RequestParam` obligatorio en `MantenimientoControlador.guardar()` (BFF) — un submit sin JS da 400 crudo de Spring en vez del mensaje amigable "Debes completar todo el checklist".
      * El endpoint backend directo (`POST /api/mantenimiento`) no deduplica `equipoIds` (el BFF sí) — filas repetidas posibles en `mantenimiento_equipo` si se llama a la API cruda.
    * Verificado en cada fase: `.\mvnw.cmd compile`/`test` en ambos módulos (backend 198/198, BFF 11/11) y `git diff --check` sin errores. Ningún commit hecho en esta sesión — los cambios quedaron en el working tree para revisión del usuario.
